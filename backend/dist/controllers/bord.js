"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.getAllBoardsController = exports.createBoardController = void 0;
const bord_1 = require("../db/models/bord");
const createBoardController = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { name } = req.body;
        const board = new bord_1.Board({
            name,
            columns: { todo: [], inProgress: [], done: [] },
        });
        yield board.save();
        res.status(201).json(board);
    }
    catch (error) {
        res.status(500).json({ message: 'Error creating board', error });
    }
});
exports.createBoardController = createBoardController;
const getAllBoardsController = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const boards = yield bord_1.Board.find().populate('columns.todo columns.inProgress columns.done');
        res.status(200).json(boards);
    }
    catch (error) {
        res.status(500).json({ message: 'Error fetching boards', error });
    }
});
exports.getAllBoardsController = getAllBoardsController;
